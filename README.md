...
3. **New database:** run `mysql/schema.sql` in full. **Existing database** (you already ran `schema.sql` before this session's changes): run `mysql/schema.sql` once, then also run each file under `mysql/migrations/` in order — they're additive (`ALTER TABLE ... ADD COLUMN`, `CREATE TABLE IF NOT EXISTS`) and safe to run against data you already have. If you're missing any of them: `0003_photo_review_queue.sql` adds the `photos` table the admin panel's photo review queue depends on, and `0004_email_events.sql` adds the table the welcome/nudge emails use to avoid double-sending.
...
- **`UPLOADS_DIR`** (required) must point somewhere that survives a rebuild. This repo builds with `output: "standalone"`, which regenerates `.next` from scratch on every `npm run build` — anything stored under `.next` or the deploy folder itself would be deleted on the next deploy. On Plesk, set this to an absolute path outside the git checkout (e.g. `/var/www/marryme-uploads`). For local dev, `./storage/uploads` is fine. It's fine if the directory itself doesn't exist yet on a fresh deploy — `src/instrumentation.ts` creates it (recursively) the moment the server starts, and `savePhoto()` also checks defensively before every write. If it can't be created (wrong path, permissions), that failure is logged loudly and specifically (`[storage] Failed to create UPLOADS_DIR at "..."`) at server startup, in `stderr.log`, rather than surfacing later as a vague upload error.
...
- **Signups** (`/admin`) — every profile, most recent first, filterable by status (pending/approved/suspended) and sortable by join date. Click a name for the full profile — every field captured at onboarding, all photos, membership, and Approve/Reject — at `/admin/users/[id]`. That detail query deliberately selects specific columns rather than `SELECT *`, so `password_hash` never leaves the database, let alone reaches the page.
...
**Manual profile review**: a completed onboarding profile now lands as `pending`, not auto-`approved` — see `MANUAL_PROFILE_REVIEW_ENABLED` in `.env.example` (defaults on). A pending profile works fully for its owner (dashboard, editing) but the matching engine's `isEligibleForNewBatch()` already required `status === 'approved'`, so it simply won't be matched until an admin approves it at `/admin/users/[id]`. Set the flag to `false` to go back to auto-approving if review can't keep up with signup volume.

## Automated emails

Two transactional emails, sent via plain SMTP (Nodemailer — see `src/lib/email.ts`), with copy in `src/lib/emailTemplates.ts`:

- **Welcome** — sent once, at the same moment a completed profile is saved and founding membership is granted (`POST /api/onboarding/profile`). `email_events` (with a unique constraint on `user_id` + `email_type`) is what actually stops it being sent twice if the form is ever resubmitted.
- **Incomplete-profile nudge** — for anyone who signed up (has an email) but never finished onboarding (`full_name IS NULL` — every onboarding field is only ever saved together, so this is a precise, cheap check), `NUDGE_DELAY_HOURS` (default 24) after they signed up. Scheduled the same way as match processing:
  ```bash
  curl -X POST https://marryme.sg/api/cron/nudge-incomplete-profiles \
    -H "Authorization: Bearer $CRON_SECRET"
  ```
  On Plesk: Scheduled Tasks → add a daily task running that `curl` command. Safe to run as often as you like — `email_events` guarantees each person is only ever nudged once.

**Without `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` set, emails are skipped with a warning logged** (same pattern as `GOOGLE_CLOUD_VISION_API_KEY`) — nothing else breaks. Get real values from Plesk: Mail → `admin@marryme.sg` → Show SMTP settings.

nodemailer is pinned to the 8.x line, not the latest 9.x, because `next-auth`'s own (unused) Nodemailer email-provider peer dependency only allows 7.x/8.x — see the comment in `src/lib/email.ts` for how the one known high-severity nodemailer advisory in that range is mitigated structurally (this app never uses the vulnerable `raw`/attachment code path) rather than by upgrading.

## What's stubbed vs. real in this MVP
- **Real:** email/password signup+login and phone OTP signup+login (via NextAuth Credentials providers against MySQL), protected `/dashboard` and `/admin` routes (via role checks), the full MySQL schema, the matching engine (batch generation, faith filtering, mutual-interest detection, cooling-off) plus an admin override on top of it, the full onboarding profile form (identity, religion, community, relationship intent, optional details, exactly 3 required photos with moderation and manual review, consent), a 12-question personality test, the "My Profile" activity page, account deletion with full cascade, a real admin panel (signup feed with full-profile vetting + approve/reject, photo review queue, match override, membership tracking), manual profile review gating who gets matched, founding-member free access auto-granted on signup, and welcome + incomplete-profile-nudge emails.
- **Stubbed:** messages page (schema is in place — no send/receive UI or real-time wiring yet), phone OTP codes log a warning and skip actually sending an SMS until an SMS provider is configured — see the Auth setup section above. There's no gate yet on `/dashboard/matches` requiring an active membership (membership rows are tracked, but nothing currently checks them before showing matches). `/privacy` and `/terms` are placeholder pages flagging that real legal copy is needed, not actual policies — don't treat them as launch-ready.
- **Deferred to post-MVP:** Singpass login (button is present but disabled — requires relying-party registration with the Singpass developer portal, which takes a few days to get approved), Stripe billing for any paid tier (memberships are tracked but not charged), verifying an email address added after a phone-only signup (no outbound email/SMTP provider is wired up for a verification flow specifically — it's stored but unverified).
