...
**Low-memory / constrained-host VPS note:** `next.config.ts` sets `experimental.cpus: 1` and `experimental.workerThreads: false` to force a single build worker — those only govern Next's own JS-level build workers, though. Turbopack's native engine has a *separate* internal thread pool (Rayon) that ignores both settings entirely and sizes itself to the number of visible CPUs. On a host with a low OS-level process/thread limit (common on shared/managed hosting with a chrooted or jailed shell), Rayon trying to spawn that many threads can hit the ceiling and crash with `EAGAIN` / `ERR_WORKER_INIT_FAILED` — which surfaces as a confusing, unrelated-looking Turbopack internal error (e.g. a CSS module chunking failure) rather than a clear "too many threads" message. `npm run build` runs through `scripts/run-build.mjs`, which sets `RAYON_NUM_THREADS=1` and `RAYON_RS_NUM_CPUS=1` before invoking `next build` to cap this — safe on any host (it only limits parallelism, never correctness), and it won't override either variable if you've already set one yourself, so a future, less-constrained host can raise it. If a build still fails after this, it's more likely genuine memory exhaustion — check the actual memory limit Plesk enforces on the Node.js application (separate from the VPS's total RAM) and raise it if possible.
